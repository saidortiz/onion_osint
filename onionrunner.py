...
def get_onion_list():
	...
	if os.path.exists("onions_list.txt"):
	
		with open("onions_list.txt","rb") as fd:
...
def store_onion(onion):
	...
	with codecs.open("onions_list.txt","ab",encoding="utf8") as fd:
		fd.write("%s\n" % onion)
...
def process_results(onion,json_response):
    ...
    print("scan_result: %s" % scan_result)
